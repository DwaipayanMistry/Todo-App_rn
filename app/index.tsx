...
export default function Index() {
    const [toDos, setToDos] = useState(data.sort((a, b) => Number(a.id) - Number(b.id)));
        ...
        const fetchData = async () => {
            try {
                ...
                if (storedTodos && storedTodos.length) {
                    setToDos(storedTodos.sort((a, b) => Number(b.id) - Number(a.id)));
                } else {
    ...
    //   checks if the font is laded or not
    if (!loaded && !error) {
        return null;
    }

    function editLink(id: string) {
        // Use template literal to format the path correctly
